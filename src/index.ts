...
class UmAlQuraCalendar {
    private static readonly _maxSeconds = 9223372036854775807 / 10000000;
    private static readonly _minSeconds = -9223372036854775807 / 10000000;

    private static readonly _millisPerSecond = 1000;
    private static readonly _millisPerMinute = UmAlQuraCalendar._millisPerSecond * 60;
    private static readonly _millisPerHour = UmAlQuraCalendar._millisPerMinute * 60;
    private static readonly _millisPerDay = UmAlQuraCalendar._millisPerHour * 24;

    private static readonly _minCalendarYear = 1318;
    private static readonly _maxCalendarYear = 1500;

    private static readonly _minDate = new Date(1900, 3, 30);
    private static readonly _maxDate = new Date(2077, 10, 16, 23, 59, 59, 999);

    private static readonly _hijriYearData = UmAlQuraCalendar._initDateMapping();
    ...
    public static hijriToGregorian(hy: number, hm: number, hd: number) {
        ...
        const index = hy - UmAlQuraCalendar._minCalendarYear;
        let dt = UmAlQuraCalendar._hijriYearData[index].gregorianDate;
        let b = UmAlQuraCalendar._hijriYearData[index].hijriMonthsLengthFlags;
    ...
    public static gregorianToHijri(time: Date) {
        UmAlQuraCalendar._checkMillsRange(time.getTime());
        ...
        let index = Math.trunc((time.getTime() - UmAlQuraCalendar._minDate.getTime()) / UmAlQuraCalendar._millisPerDay / 355);

        do {
        } while (time.getTime() > UmAlQuraCalendar._hijriYearData[++index].gregorianDate.getTime());

        if (time.getTime() !== UmAlQuraCalendar._hijriYearData[index].gregorianDate.getTime()) {
            index--;
        }

        let nDays = UmAlQuraCalendar._dayDiff(time, UmAlQuraCalendar._hijriYearData[index].gregorianDate);
        const yh1 = index + UmAlQuraCalendar._minCalendarYear;
        let mh1 = 1;
        let dh1 = 1;
        let b = UmAlQuraCalendar._hijriYearData[index].hijriMonthsLengthFlags;
    ...
    public static addYears(time: Date, years: number) {
        return UmAlQuraCalendar.addMonths(time, years * 12);
    }

    public static addMonths(time: Date, months: number) {
        // Get the date in UmAlQura calendar.
        let y = UmAlQuraCalendar._getDatePart(time, DatePart.Year);
        let m = UmAlQuraCalendar._getDatePart(time, DatePart.Month);
        let d = UmAlQuraCalendar._getDatePart(time, DatePart.Day);
        const i = m - 1 + months;
        ...
        return new Date(UmAlQuraCalendar._getAbsoluteDateUmAlQura(y, m, d) * UmAlQuraCalendar._millisPerDay + time.getTime() % UmAlQuraCalendar._millisPerDay);
    }

    public static getDayOfYear(time: Date) {
        return UmAlQuraCalendar._getDatePart(time, DatePart.DayOfYear);
    }

    public static gGetDayOfMonth(time: Date) {
        return UmAlQuraCalendar._getDatePart(time, DatePart.Day);
    }

    public static getDayOfWeek(time: Date) {
        return time.getDay();
    }
    ...
    public static getDaysInYear(year: number) {
        ...
        let days = 0;
        let b = UmAlQuraCalendar._hijriYearData[year - UmAlQuraCalendar._minCalendarYear].hijriMonthsLengthFlags;
    ...
    public static getDaysInMonth(year: number, month: number) {
        ...
        if ((UmAlQuraCalendar._hijriYearData[year - UmAlQuraCalendar._minCalendarYear].hijriMonthsLengthFlags & (1 << month - 1)) == 0) {
            return 29;
        }
        else {
            return 30;
    ...
    public static getYear(time: Date) {
        return UmAlQuraCalendar._getDatePart(time, DatePart.Year);
    }

    public static getMonth(time: Date) {
        return UmAlQuraCalendar._getDatePart(time, DatePart.Month);
    }
    ...
    public static toDate(year: number, month: number, day: number, hour: number = 0, minute: number = 0, second: number = 0, millisecond: number = 0) {
        ...
        return new Date(lDate * UmAlQuraCalendar._millisPerDay + UmAlQuraCalendar._timeToMillis(hour, minute, second, millisecond));
    }

    private static _getDatePart(time: Date, part: DatePart) {
        const { hy, hm, hd } = UmAlQuraCalendar.gregorianToHijri(time);
    ...
    private static _getAbsoluteDateUmAlQura(year: number, month: number, day: number) {
        ...
        return new Date(gy, gm, gd).getTime() / UmAlQuraCalendar._millisPerDay;
    }

    private static _timeToMillis(hour: number, minute: number, second: number, millisecond: number) {
        if (hour < 0 || hour >= 24 || minute < 0 || minute >= 60 || second < 0 || second >= 60 || millisecond < 0 || millisecond >= UmAlQuraCalendar._millisPerSecond) {
            throw new Error('Invalid value for hour, minute, second or millisecond.');
        }

        const totalSeconds = hour * 3600 + minute * 60 + second;
        if (totalSeconds > UmAlQuraCalendar._maxSeconds || totalSeconds < UmAlQuraCalendar._minSeconds) {
            throw new Error('Time overflow error. This is possibly a bug.');
        }

        return (totalSeconds * UmAlQuraCalendar._millisPerSecond) + millisecond;
    }

    private static _checkYearRange(year: number) {
        if (year < UmAlQuraCalendar._minCalendarYear || year > UmAlQuraCalendar._maxCalendarYear) {
            throw new Error(`Invalid value for year. Must be between ${UmAlQuraCalendar._minCalendarYear} and ${UmAlQuraCalendar._maxCalendarYear}.`);
        }
    }
    ...
    private static _checkMillsRange(millis: number) {
        if (millis < UmAlQuraCalendar._minDate.getTime() || millis > UmAlQuraCalendar._maxDate.getTime()) {
            throw new Error(`Invalid value for epoch. Must be between ${UmAlQuraCalendar._minDate.getTime()} and ${UmAlQuraCalendar._maxDate.getTime()}.`);
        }
    }

    private static _addDays(d: Date, days: number) {
        const date = new Date(d.valueOf());
        date.setDate(date.getDate() + days);
        return date;
    }

    private static _dayDiff(d: Date, other: Date) {
        return Math.round((d.getTime() - other.getTime()) / (1000 * 60 * 60 * 24));
    }
